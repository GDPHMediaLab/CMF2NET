...
if __name__ == "__main__":
    ...
    task_name = "Taskxxx_DCET2Reg"
    downloaded_data_dir_dce = "/xxxxx/CMF2Net/data/nnUNet_raw_data/Taskxxx_DCE/images"
    downloaded_data_dir_t2 = "/xxxxx/CMF2Net/data/T2_to_DCE/Taskxxx_T2/images"

    downloaded_data_dir_dce_test = "/xxxxx/CMF2Net/data/nnUNet_raw_data/Taskxxx_DCE/imagesTs"
    downloaded_data_dir_t2_test = "/xxxxx/CMF2Net/data/nnUNet_raw_data/Taskxxx_T2/imagesTs"
    ...
    dce_paths = os.listdir(downloaded_data_dir_dce)
    dce_test_paths = os.listdir(downloaded_data_dir_dce_test)

    ## trainset
    train_val_patient_names = []
    for p in tqdm(dce_paths):
        dce = join(downloaded_data_dir_dce, os.path.basename(p))
        t2 = join(downloaded_data_dir_t2, os.path.basename(p).replace('DCE', 'T2'))
        dce_seg = join(downloaded_data_dir_dce.replace('images', 'labels'),os.path.basename(p).replace('_0000',''))
        # t2_seg = join(downloaded_data_dir_t2.replace('images', 'labels'),os.path.basename(p).replace('DCE', 'T2'))

        target_dce = join(target_imagesTr, os.path.basename(dce).replace('_DCE_','_'))
        target_t2 = join(target_imagesTr, os.path.basename(t2).replace('_T2_', '_').replace('0000', '0001'))
        target_dce_seg = join(target_labelsTr, os.path.basename(dce_seg).replace('_DCE_','_'))
        # target_t2_seg = join(target_labelsTr,os.path.basename(dce_seg).replace('DCE', 'T2'))

        if not os.path.exists(target_dce):
            shutil.copy(dce, target_dce)
        if not os.path.exists(target_t2):
            shutil.copy(t2, target_t2)
        if not os.path.exists(target_dce_seg):
            copy_DCE_T2_segmentation_and_convert_labels(dce_seg, target_dce_seg)
        # if not os.path.exists(target_t2_seg):
        #     copy_DCE_T2_segmentation_and_convert_labels(t2_seg, target_t2_seg)

        train_val_patient_names.append(p.replace('_DCE_','_').replace('_0000',''))

    ## testset
    test_patient_names = []
    for p in tqdm(dce_test_paths):
        dce = join(downloaded_data_dir_dce_test, os.path.basename(p))
        t2 = join(downloaded_data_dir_t2_test,os.path.basename(p).replace('DCE', 'T2'))
        dce_seg = join(downloaded_data_dir_dce_test.replace('imagesTs', 'labelsTs'),os.path.basename(p).replace('_0000',''))
        # t2_seg = join(downloaded_data_dir_t2_test.replace('imagesTs', 'labelsTs'),os.path.basename(p).replace('DCE', 'T2').replace("_0000",""))

        target_dce = join(target_imagesTs, os.path.basename(dce).replace('_DCE_', '_'))
        target_t2 = join(target_imagesTs, os.path.basename(t2).replace('_T2_', '_').replace('0000', '0001'))
        target_dce_seg = join(target_labelsTs, os.path.basename(dce_seg).replace('_DCE_', '_'))
        # target_t2_seg = join(target_labelsTs, os.path.basename(t2_seg))
        ...
        test_patient_names.append(p.replace('_DCE_','_').replace('_0000',''))


    json_dict = OrderedDict()
    json_dict['name'] = task_name
    json_dict['description'] = "MM DCE Segment"
    json_dict['tensorImageSize'] = "4D"
    json_dict['reference'] = "see DCET2"
    json_dict['licence'] = "see DCET2 license"
    ...
    json_dict['numTraining'] = len(train_val_patient_names)
    # json_dict['numTest'] = len(test_patient_names)
    json_dict['numTest'] = 0
    json_dict['training'] = [{'image': "./images/%s" % i, "label": "./labels/%s" % i} for i in
                             train_val_patient_names]
    json_dict['test'] = []
    # json_dict['test'] = [{'image': "./imagesTs/%s" % i, "label": "./labelsTs/%s" % i} for i in
    #                      test_patient_names]
